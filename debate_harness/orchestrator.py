...
class Orchestrator:
    ...
    def present(
        ...
        user = (
            ...
            f"{stop_reason}). Write the FINAL ANSWER for the user — the best, most "
            "useful standalone answer the debate supports. It must be COMPLETE and "
            "decision-useful, NOT a brief summary or a single paragraph: preserve "
            "the depth the debate produced. Specifically:\n"
            "- Take the position the debate lands on (or, if it genuinely did not "
            "converge, lead with the irreducible tradeoff).\n"
            "- Give the load-bearing reasons, organized into the distinct dimensions "
            "the debate actually worked through.\n"
            "- Integrate the strongest points AND the concessions from BOTH sides — "
            "what each side conceded and why.\n"
            "- State the concrete conditions under which the answer would change.\n"
            "If the models reached genuine consensus, present the agreed answer in "
            "full. If a productive stalemate, present the answer WITH the residual "
            "tradeoff stated cleanly and each side's reasoning. If they did not "
            "converge, present the best available synthesis and clearly flag the "
            "non-convergence. Do not inject your own position or take a side beyond "
            "what the debate supports.\n\n"
